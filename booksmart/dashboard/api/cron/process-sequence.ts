...
export default withRunLog("cron.process-sequence", async (_req, res) => {
  const now = new Date();
  if (now < FIRST_RUN_AT) {
    res.status(200).json({ skipped: true, reason: "before_first_run" });
    return;
  }
  ...
  if (remaining <= 0) {
    res.status(200).json({ skipped: true, reason: "daily_cap_reached", sentToday });
    return;
  }
  ...
  if (!step || !lead) {
    ...
    res.status(200).json({ skipped: true, reason: "no_step_or_lead" });
    return;
  }
